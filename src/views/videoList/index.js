...
export default class VideoList extends Component {
  ...
  getTbodyData(videoList) {
    ...
    let tbodyData = [];
    videoList.forEach(vInfo => {
      tbodyData.push({
        thumbnail: <img src={vInfo.first_image} alt="" />,
  ...
  processVideoListData(videoListData) {
    ...
    let total = parseInt(videoListData.message);
    ...
    } else if (total <= 0 || !videoList || videoList.length === 0) {
      sysInfo = '没有更多信息！';
  ...
  handlePageControlClick(name) {
    ...
    this.fetchVideoList(curPageNum, this.state.searchKeyword);
